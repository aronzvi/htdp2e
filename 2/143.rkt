...
; (average empty) produces "/: division by zero" error

;; List-of-temperatures -> Number
;; computes the average tempature. produces an error when list is empty
(check-error (checked-average empty))
(check-expect(checked-average (cons 1 (cons 2 (cons 3 '())))) 2)

(define (checked-average lot)
  (if (empty? lot)
      (error "checked-average: tempature list must not be empty")
      (average lot)))
