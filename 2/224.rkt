...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname |224|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (missiles-render m img)
  (cond
    [(empty? m) img]
    [else (missile-render (first m)       
                          (missiles-render (rest m) img))])) 
...
(define (render-fired-charges loc i)
  (cond
    [(empty? loc) i]
    [else (charge-render (first loc) 
                         (render-fired-charges (rest loc) i))]))
...
(define (missiles-move lom)
  (cond
    [(empty? lom) empty]
    [else (cons (missile-move (first lom))        
                (missiles-move (rest lom)))])) 
...
;(define (remove-off-canvas-missiles lom) lom) ;stub

(define (remove-off-canvas-missiles lom)
  (cond
    [(empty? lom) empty]
    [else
     (if (missile-on-canvas? (first lom))
         (cons (first lom) (remove-off-canvas-missiles (rest lom)))
         (remove-off-canvas-missiles (rest lom)))]))
...
(define (remove-off-canvas-fired-charges loc)
  (cond
    [(empty? loc) empty]
    [else (if (charge-on-canvas? (first loc))
              (cons (first loc) (remove-off-canvas-fired-charges (rest loc)))
               (remove-off-canvas-fired-charges (rest loc)))]))
...
(define (move-fired-charges loc)
  (cond
    [(empty? loc) empty]
    [else (cons (charge-move (first loc))    
                (move-fired-charges (rest loc)))]))
...
;(define (missiles-hit-ufo? u lomm) false) ; stub

(define (missiles-hit-ufo? u lom)
  (cond
    [(empty? lom) false]
    [else (or (missile-hit-ufo? u (first lom))       
              (missiles-hit-ufo? u (rest lom)))]))

;; Ufo Missile -> Boolean
;; produces true if distance between missile and UFO <= HIT-PROXIMITY
(check-expect (missile-hit-ufo? (make-posn 30 70) (make-posn 80 80)) false)
(check-expect (missile-hit-ufo? (make-posn 30 70) (make-posn 30 (- 70 HIT-PROXIMITY))) true)
(check-expect (missile-hit-ufo? (make-posn 30 70) (make-posn 30 (- 70 (- HIT-PROXIMITY 0.5)))) true)
...
(define (charges-fired-hit-tank? t loc)
  (cond
    [(empty? loc) false]
    [else (or (charge-hit-tank? t (first loc))     
               (charges-fired-hit-tank? t (rest loc)))]))

;; Tank Charge -> Boolean
;; produces true if distance between charge and tank <= TANK-HIT-PROXIMITY
(check-expect (charge-hit-tank? (make-tank 30 5) (make-posn 80 80)) false) ; charge far away             
(check-expect (charge-hit-tank? (make-tank 30 5) (make-posn 30 TANK-Y)) true) ; hit. exact same
(check-expect (charge-hit-tank? (make-tank 30 5) (make-posn 30 (- TANK-Y TANK-HIT-PROXIMITY))) true) ; hit. distance equals TANK-HIT-PROXIMITY
(check-expect (charge-hit-tank? (make-tank 30 5) (make-posn 30 (- TANK-Y (- TANK-HIT-PROXIMITY 0.5)))) true)  ; hit. distance less than TANK-HIT-PROXIMITY

;(define (charge-hit-tank? t c) false) ; stub

; Template from ?

(define (charge-hit-tank? t c)
