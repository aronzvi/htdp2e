...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |521|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define INVISIBLE-PERSON (circle PERSON-R "solid" "white"))
...
(define-struct puzzle-state [num-mc-left num-mc-right boat-location])
; A PuzzleState is a structure:
; (make-puzzle-state NumMissionariesAndCannibals NumMissionariesAndCannibals BoatLocation)
...
(define PS-INIT (make-puzzle-state (make-num-missionaries-cannibals MAX-PEOPLE-IN-GROUP MAX-PEOPLE-IN-GROUP)
                                   ...
                                   "left"))
(define PS-INTER (make-puzzle-state (make-num-missionaries-cannibals 1 1)
...
(check-expect (final? PS-INTER) #false)
...
(check-expect (render-mc PS-INTER) (beside (render-river-side-and-people (make-num-missionaries-cannibals  1 1))
                                           (render-river-and-boat "left")
                                           (render-river-side-and-people (make-num-missionaries-cannibals 2 2))))
...
(check-expect (render-river-side-and-people (make-num-missionaries-cannibals 0 MAX-PEOPLE-IN-GROUP))
              (overlay (beside/align "top"
                                     ...
                                     (render-group MAX-PEOPLE-IN-GROUP CANNIBAL))
                       RIVER-SIDE))
(check-expect (render-river-side-and-people (make-num-missionaries-cannibals MAX-PEOPLE-IN-GROUP 0))
              (overlay (beside/align "top"
                                     (render-group MAX-PEOPLE-IN-GROUP MISSIONARY)
                                     SPACER
                                     INVISIBLE-PERSON)
...
(define (render-river-side-and-people mc)
  (local ((define missionaries-img (render-group (num-missionaries-cannibals-m mc) MISSIONARY))
          (define cannibals-img (render-group (num-missionaries-cannibals-c mc) CANNIBAL)))
    (overlay (beside/align  "top"
                            (if (equal? missionaries-img empty-image)
                                INVISIBLE-PERSON
                                missionaries-img)
                            SPACER
                            (if (equal? cannibals-img empty-image)
                                INVISIBLE-PERSON
                                cannibals-img))
             RIVER-SIDE)))
