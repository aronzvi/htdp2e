...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |344|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require htdp/dir)
...
(check-expect (find-all TEST-DIR "xxxxz") empty)
(check-expect (find-all TEST-DIR ".DS_Store")
              '(("testdir" ".DS_Store")
                ("testdir" "one" ".DS_Store")
                ("testdir" "one" "1" ".DS_Store")
                ("testdir" "one" "2" ".DS_Store")))
(check-expect (find-all TEST-DIR "f-1-2-2.txt")
              '(( "testdir" "one" "2" "f-1-2-2.txt")))
