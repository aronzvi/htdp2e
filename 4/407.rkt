...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |407|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (project.v1 db labels)
  (local ((define schema  (db-schema db))
          ...
          (define (row-project row)
            (foldr (lambda (c n res) (if (member? n labels) (cons c res) res))
                   empty
                   row
                   schema-names)))
