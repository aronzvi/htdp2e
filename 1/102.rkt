...
;; SIGS -> SIGS
;; start the world with (main (make-sigs (make-posn SCENE-MID-X 0) (make-tank SCENE-MID-X TANK-SPEED) false))
;; 
(define (main s)
  (big-bang s                                      ; SIGS
...
(define (si-game-over? s)
  (or (ufo-landed? (sigs-ufo s))           
      (missile-hit-ufo? (sigs-missile s) (sigs-ufo s))))
...
(check-expect (missile-hit-ufo? #false (make-posn 30 70)) false) ; missile not fired
...
(define (missile-hit-ufo? m u)
  (cond
    [(false? m) false]
    [else (and (<= (missile-top m) (posn-y u))
...
;; MissileorNot -> Number
;; returns the top y value for the given missile using the MISSILE image. Assumes that missile is not #false
