...
(define (quick-sort< alon)
  (cond
    [(empty? alon) '()]
    [else (local ((define pivot (first alon))
                  (define alon-without-pivot (filter (lambda (n) (not (equal? n pivot))) alon)))
            (append (quick-sort< (smallers alon-without-pivot pivot))
                    (list pivot)
                    (quick-sort< (largers alon-without-pivot pivot))))]))
